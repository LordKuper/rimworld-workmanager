...
- [x] `WorkShift`: test start/end hour-to-shift mapping and validation/normalization of out-of-range or invalid hour values. (AC-2)
...
- [x] `PassionHelper.GetPassionScore`: test normalization across all handled passion levels, asserting the expected normalized score for each. (AC-5)
